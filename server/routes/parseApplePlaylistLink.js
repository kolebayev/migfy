...
router.post("/parseApplePlaylistLink", async (req, res) => {
  ...
  let artistNodes = dom.window.document.querySelectorAll(
    "div.typography-caption > a.dt-link-to"
  );
  ...
  let plCoverURL = urlArray[urlArray.length - 1].substring(0, urlArray[urlArray.length - 1].length-5);

  let plName = dom.window.document
  ...
  for (let i = 0; i < trackNodes.length; i++) {
    ...
    url.length > 1
      ? (url = url.split(",")[1].slice(0, -4).replace(/\n/g, ""))
      : (url = "");
